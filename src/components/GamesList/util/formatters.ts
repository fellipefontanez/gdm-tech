...
export const associarOdds = (
  participantKey: string,
  outcomes: Outcome[],
  index: number
): string => {
  const exactMatch = outcomes.find(
    out => out.participantKey === participantKey
  );

  if (exactMatch) return formatarOdds(exactMatch.payout);

  const similarMatch = outcomes.find(out =>
    out.participantKey.startsWith(participantKey.slice(0, 4))
  );
