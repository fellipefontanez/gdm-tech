...
const BarraDeFavoritos = () => {
  const { itens, isOpen, setIsOpen, isHovered, setIsHovered, handleRemove, reorderList, showOnlyFavCategories, setShowOnlyFavCategories } =
    useBarraDeFavoritos();
        ...
        {isOpen && (
          <motion.div
            ...
            className={`fixed z-40 bg-white shadow-md p-4 ${
              isMobile ? "top-[74px] left-0 w-full rounded-md md:hidden" : "top-[74px] left-0 w-[280px] h-full rounded-r-md hidden md:block"
            }`}
          >
            ...
            <button
              ...
              className={`mt-8 w-full text-sm font-medium px-4 py-2 rounded-md text-white transition-colors duration-200 ${
                showOnlyFavCategories ? "bg-emerald-600 hover:bg-emerald-500" : "bg-neutral-800 hover:bg-neutral-700"
              }`}
            >
