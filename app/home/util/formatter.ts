import {
  AdvantagesV1Response,
  Vantagem,
} from '@/src/types/eventResponse.model';
...
export async function getListaDadosGerais(data?: AdvantagesV1Response) {
  ...
  const vantagensFiltradas = allVantagens.filter(v =>
    esportesAlvo.includes(v.context.eventSport)
  );
  ...
  for (const esporte of esportesAlvo) {
    ...
    const unicosPorEvento = _.uniqBy(eventos, v => v.context.eventKey).slice(
      0,
      3
    );
...
export async function getListaDadosPorEsporte(
  esporte: string,
